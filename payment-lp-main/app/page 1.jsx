  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {selectedRoom && (
        <div className="fixed bottom-6 left-6 z-40 md:hidden">
          <button
            ...
            className="bg-amber-700 hover:bg-amber-800 text-white p-4 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110"
      ...
      >
        ...
        <div className="absolute inset-0 block md:hidden pointer-events-none z-10" style={{ background: 'rgba(236, 247, 93, 1)' }}></div>
        ...
        >
          <div className="flex justify-between items-center p-4 border-b border-amber-100">
            <div className="flex items-center">
              <img src="لوغو مآثر-01-01.png" alt="شركة ماثر" className="w-12 h-12" />
            </div>
            <button
              className="p-2 rounded focus:outline-none focus:ring-2 focus:ring-amber-400"
        ...
        <div className="flex flex-col md:flex-row justify-between items-center w-full max-w-7xl mx-auto px-10 py-16 md:py-24">
          <div className="flex-1 flex flex-col items-start text-right dir-rtl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <a href="#rooms" className="bg-amber-700 hover:bg-amber-800 text-white font-bold py-3 px-8 rounded-lg transition text-lg shadow-lg">
                اختر غرفتك
              </a>
              <a href="#sheikh" className="bg-transparent border-2 border-white hover:bg-white hover:text-amber-900 text-white font-bold py-3 px-8 rounded-lg transition text-lg">
                تعرف على الشيخ
              </a>
            </div>
          </div>
      ...
      <section className="py-16 px-4 bg-gradient-to-b from-stone-50 to-white" id="rooms">
        <div className="max-w-6xl mx-auto text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-amber-900">أنواع الغرف المتاحة</h2>
          ...
          {ROOM_TYPES.map((room) => (
            <div
              ...
              className={`bg-white rounded-xl shadow-lg p-6 flex flex-col items-center border-2 transition-all duration-300 ${
                form.roomType === room.id ? 'border-amber-500 bg-amber-50' : 'border-stone-200 hover:border-amber-300'
              }`}
              style={{ minWidth: '0', maxWidth: '100%' }}
            >
              <div className="text-6xl mb-4">{room.icon}</div>
              <h3 className="font-bold text-2xl mb-2 text-center text-amber-900">{room.name}</h3>
              <div className="text-stone-600 text-md mb-4 text-center leading-relaxed">
                <div className="mb-2">{room.description}</div>
              </div>
              <div className="font-bold text-amber-700 text-3xl mb-4">{room.price} {room.currency}</div>
            ...
            {ROOM_TYPES.map((room) => (
              <div
                ...
                className={`bg-white rounded-xl shadow-lg p-8 flex flex-col items-center border-2 transition-all duration-300 hover:shadow-xl ${
                  form.roomType === room.id ? 'border-amber-500 bg-amber-50 transform scale-105' : 'border-stone-200 hover:border-amber-300'
                }`}
              >
                <div className="text-8xl mb-6">{room.icon}</div>
                <h3 className="font-bold text-2xl mb-4 text-center text-amber-900">{room.name}</h3>
                <div className="text-stone-600 text-lg mb-6 text-center leading-relaxed">
                  <div className="mb-2">{room.description}</div>
                </div>
                <div className="font-bold text-amber-700 text-4xl mb-2">{room.price} {room.currency}</div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="text-right dir-rtl">
              <h2 className="text-3xl md:text-4xl font-bold mb-6 text-amber-900">الشيخ المرافق</h2>
            ...
            <div className="flex justify-center">
              <div className="relative">
                <img 
                  ...
                  className="w-80 h-80 object-cover rounded-full shadow-2xl border-8 border-amber-100"
                />
      ...
      <section className="py-16 px-4 bg-gradient-to-b from-stone-50 to-white" id="program">
        <div className="max-w-6xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-6 text-amber-900">ما يشمله العرض</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
              ...
              <h3 className="text-xl font-bold mb-3 text-amber-900">الطيران والتأشيرة</h3>
              <p className="text-stone-600">تذاكر الطيران ذهاباً وإياباً + التأشيرة</p>
            </div>
            
            <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
              <div className="text-4xl mb-4">🏨</div>
              <h3 className="text-xl font-bold mb-3 text-amber-900">الإقامة</h3>
              <p className="text-stone-600">فنادق 4 نجوم مع جميع وسائل الراحة</p>
            </div>
            
            <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
              <div className="text-4xl mb-4">🍽️</div>
              <h3 className="text-xl font-bold mb-3 text-amber-900">الوجبات</h3>
              <p className="text-stone-600">وجبة الفطور يومياً</p>
            </div>
            
            <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
              <div className="text-4xl mb-4">🕌</div>
              <h3 className="text-xl font-bold mb-3 text-amber-900">الزيارات</h3>
              <p className="text-stone-600">زيارات المآثر النبوية والأماكن المقدسة</p>
            </div>
            
            <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
              <div className="text-4xl mb-4">🚌</div>
              <h3 className="text-xl font-bold mb-3 text-amber-900">التنقلات</h3>
              <p className="text-stone-600">جميع التنقلات بحافلات مكيفة ومريحة</p>
            </div>
            
            <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
              <div className="text-4xl mb-4">🎯</div>
              <h3 className="text-xl font-bold mb-3 text-amber-900">البرامج</h3>
...
<section className="py-16 px-4 bg-white" id="previous-trips">
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 max-w-6xl mx-auto">
    ...
    <img src="الرحل السابقة 2.jpgc:\Users\LENOVO\Downloads\page_modified.jsx" alt="رحلة 4" className="w-full h-64 object-cover rounded-lg shadow-lg hover:scale-105 transition-transform" />
  </div>
</section>
      ...
      <section className="py-16 px-4 bg-stone-100" id="booking-form">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-amber-900">احجز مكانك الآن</h2>
            <p className="text-lg text-stone-600">املأ بياناتك لتأكيد حجزك في عمرة الإرث النبوي</p>
          </div>
          
          {/* Selected Room Summary */}
          {form.roomType && (
            <div className="mb-8 bg-white p-6 rounded-xl shadow-lg border-l-4 border-amber-500">
              <h3 className="text-xl font-bold mb-4 text-amber-900">تفاصيل الحجز</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <span className="font-semibold text-stone-700">نوع الغرفة:</span>
                  <span className="mr-2 text-amber-700">{ROOM_TYPES.find(r => r.id === form.roomType)?.name}</span>
                </div>
                <div>
                  <span className="font-semibold text-stone-700">السعر:</span>
                  <span className="mr-2 text-amber-700">{ROOM_TYPES.find(r => r.id === form.roomType)?.price} دولار للشخص</span>
                </div>
                <div>
                  <span className="font-semibold text-stone-700">عدد الأشخاص:</span>
                  <span className="mr-2 text-amber-700">{form.numberOfPeople}</span>
                </div>
                <div>
                  <span className="font-semibold text-stone-700">المجموع:</span>
                  <span className="mr-2 text-2xl font-bold text-amber-700">
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg">
            <h3 className="text-2xl font-bold mb-6 text-amber-900">المعلومات الشخصية</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="animate-slideIn" style={{ animationDelay: '0.5s' }}>
                ...
                <textarea
                  ...
                  rows="4"
                  className="w-full p-3 border border-stone-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all"
                />
              </div>
              
              <button 
                type="submit" 
                className="w-full bg-amber-700 hover:bg-amber-800 text-white font-bold py-4 px-8 rounded-lg transition-all duration-200 transform hover:scale-[1.02] hover:shadow-lg animate-slideIn text-lg"
      ...
      {/* Footer */}
      <footer className="bg-amber-900 text-white pt-12 pb-4 px-4 mt-auto">
        <div className="max-w-6xl mx-auto">
